...
public function get_vat_number($user_id)
{
 ...
 public function bulk_switch_roles($roles_array)
 {
  ...
  public function get_user_total_spent($starting_date = null, $ending_date = null, $orders = array(),  $currency_symbol = true)
	...
	//Normal flow
	public function get_all_users_with_orders_ids($current_page, $per_page, $customers_ids_to_use_as_filter = null, $get_total = false, $ids = null, $starting_date = null, $ending_date = null, $filter_by_product_id = null, $is_for_export = false)
	{
		...
		else if(isset($_GET['orderby']) && isset($_GET['order']))
		{
			$order =  $_GET['order'];
	...
	public function update_user_metas( $user_id, $data_source)
	{
		...
		$country_helper = new WCCM_Country();
		$data_source['Billing country'] = isset($data_source['Billing country']) ? $country_helper->get_country_code_by_name($data_source['Billing country']) : null;
		$data_source['Billing state'] = isset($data_source['Billing state']) ? $country_helper->get_state_code_by_name($data_source['Billing country'], $data_source['Billing state']) : null;
		$data_source['Shipping country'] = isset($data_source['Shipping country']) ? $country_helper->get_country_code_by_name($data_source['Shipping country']) : null;
		$data_source['Shipping state'] = isset($data_source['Shipping state']) ? $country_helper->get_state_code_by_name($data_source['Shipping country'], $data_source['Shipping state']) : null;
