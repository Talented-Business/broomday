		...
		<div class="wrap">
			...
			<div id="options-container">
				...
				<p>
					...
					<input type="number" min=1 name="wccm_general_options[map_default_zoom_level]" value="<?php if(isset($options['map_default_zoom_level'])) echo $options['map_default_zoom_level']; else echo '15' ?>" ></input>
				</p> 
				...
				<p style="margin-top:30px;">
					<label><?php _e('New user email subject','woocommerce-customers-manager');?></label>
					<p><?php _e('Leave empty to use default text.');?></p>
					<input type="text" name="wccm_general_options[email_new_user_subject]" placeholder="<?php echo __('Default: New account','woocommerce-customers-manager') ?>" value="<?php if(isset($options['email_new_user_subject'])) echo $options['email_new_user_subject']; ?>" ></input>
				</p>
				<p style="margin-top:30px;">
					<label><?php _e('New user email body','woocommerce-customers-manager');?></label>
					<p><?php _e('Leave empty to use the default text. Use the %login and %password placeholders to print the user login and password inside the text.');?></p>
					<!--<textarea id="wccm_email_new_user_body" type="text" name="wccm_general_options[email_new_user_body]" placeholder="<?php echo __('Default: New account has been created. Login using the following credentials.<br/>User: %login <br/>Password: %password<br/>','woocommerce-customers-manager') ?>"><?php if(isset($options['email_new_user_subject'])) echo $options['email_new_user_subject']; ?></textarea>-->
					
					<?php 
						$content = isset($options['email_new_user_body']) ? $options['email_new_user_body'] : "New account has been created. Login using the following credentials.<br/>User: %login <br/>Password: %password<br/>";
					 wp_editor( $content, "email_new_user_body", array('media_buttons' => false, 'wpautop ' => false, 'textarea_name' => 'wccm_general_options[email_new_user_body]')); ?>
				</p>
				<div class="spacer"></div>
				...
				<h3 class="wccm_section_title wccm_no_margin_top"><?php _e('Manual guest conversion - Order/Guests list page', 'woocommerce-customers-manager');?></h3>
				<p>
				...
				<h3 class="wccm_section_title wccm_no_margin_top"><?php _e('Action triggering', 'woocommerce-customers-manager');?></h3>
				<p>
				...
				<h3 class="wccm_section_title wccm_no_margin_top"><?php _e('WPUEF options', 'woocommerce-customers-manager');?></h3>
				<p>
