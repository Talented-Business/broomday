...
class WCCM_CustomerDetails
{
	...
	var $is_guest_customer;
	 ...
	 function __construct()
	 {
		...
		add_action('wp_footer', array(&$this, 'include_footer_scripts'));
	 }
 
	 function include_footer_scripts()
	 {
		 ?>
		 <!-- <script  type="module" src="<?php echo WCCM_PLUGIN_PATH; ?>/js/maps/leaflet-geosearch-master/src/index.js"></script>
		 <script  type="module" src="<?php echo WCCM_PLUGIN_PATH; ?>/js/maps/leaflet-geosearch-master/src/providers/index.js"></script>
		 <script  type="module" src="<?php echo WCCM_PLUGIN_PATH; ?>/js/admin-map-module.js"></script>-->
		<?php 
	 }
	 ...
	 private function send_notification_mail()
	 {
		 ...
		 $mail->trigger($_POST['customer_mail'], $_POST['mail_subject'], $_POST['mail_text'], "notification", $user->data->ID);
	 ...
	 function render_page()
	 {
		...
		<?php 
				...
				wp_enqueue_style('wccm-leaflet',WCCM_PLUGIN_PATH.'/css/maps/leaflet.css');   
				...
				wp_enqueue_script('wccm-leaflet', WCCM_PLUGIN_PATH.'/js/maps/leaflet.js', array('jquery'));
				wp_register_script('wccm-map', WCCM_PLUGIN_PATH.'/js/admin-customer-details-map.js', array('jquery'));
	...
	?>
		<div id="user-general-details">
			...
			<p>
				<!-- <label><?php  _e('Profile Image', 'woocommerce-customers-manager'); ?></label>-->
				
				<div class="user_info">
				<div class="user_image">
					<?php echo get_avatar($this->current_customer_id, 90, "", false, array('class'=>'wccm_avatar_img')); ?>
				</div>
				<div class="user_data">
					<h3 class="customer_name">
						<?php echo $this->customer_extra_info['first_name'][0].' '.$this->customer_extra_info['last_name'][0]; ?>
					</h3>
					<p class="orderandother">
					<ul class="order-total-login">
						<li>
						...
						<li>
							...
							<span><?php echo WCCM_CustomerDetails::get_user_total_spent(null,null, $this->get_all_user_orders($this->current_customer_id)); ?></span>
						</li>
						<li>
							<span class="dashicons dashicons-calendar-alt"></span>
							<span><?php
							$last_login = get_the_author_meta('last_logins', $this->current_customer_id);
							echo $the_login_date = date('M j, Y h:i a', $last_login);
							?></span>
						</li>
					</ul>
			...
			<div id="orders-list" style="display: none">
				...
				<?php if(isset($_GET['customer'])&&false):  ?>
				...
				<?php 
					...
					<div class="order-block">
						...
						?></span></span>
					<!--	<span class="stats"><span class="/"><?php //_e('Sub total:', 'woocommerce-customers-manager' ); ?></span> <span class="stats_content"><?php echo get_woocommerce_currency_symbol().round($order_total,2); ?></span></span>
			...
			<div id="order-list-paging" style="display: none"></div>
		...
		<div id="user-geo-details-and-stats">
				...
				<?php if(WCCM_Options::get_option('hide_map_on_customer_detail_page') !== 'true'): 
					...
					$js_map_options = array(
						...
						'zoom_level' => WCCM_Options::get_option('map_default_zoom_level', 8)
					);
					wp_localize_script( 'wccm-map', 'wccm_map', $js_map_options );
					wp_enqueue_script( 'wccm-map' ); 
	...
	<div id="postbox-container-1" class="postbox-container">
		<div id="side-sortables" class="meta-box-sortables ui-sortable" style="">
			<div id="postimagedivnote" class="postbox ">
				<button type="button" class="handlediv" id="customer_note" aria-expanded="true">
					<span class="screen-reader-text">Note:</span>
					<span class="toggle-indicator" aria-hidden="true"></span>
				</button>
				<h2 class="hndle ui-sortable-handle"><span>Customer Note: </span></h2>
				<div class="inside">
					<?php if(!$this->is_guest_customer): ?>
						<div id="notes-box">
							<!--<h3 class="wccm_title_with_distance"><?php  _e('Notes', 'woocommerce-customers-manager'); ?></h3>-->
							<p>
								<form name="customer_notes" method="post" action="">
									<input type="hidden" value="<?php echo $this->current_customer_id ?>" name="customer_id">
									<!--<label><?php  _e('Notes', 'woocommerce-customers-manager'); ?></label> -->
									<textarea id="customer_notes"  cols="40" rows="4" name="customer_notes"><?php if(isset($this->customer_extra_info['wccm_customer_notes'])) echo $this->customer_extra_info['wccm_customer_notes'][0];?></textarea>
									<p class="submit">
									<input class="button-primary" type="submit" value=" <?php _e('Save notes', 'woocommerce-customers-manager' ); ?>"> </input>
									</p>
								</form>
		...
		<div id="normal-sortables" class="meta-box-sortables ui-sortable" style="">
			<div id="postimagedivnote" class="postbox ">
				<button type="button" class="handlediv" id="customer_order" aria-expanded="true">
					<span class="screen-reader-text">Order:</span>
					<span class="toggle-indicator" aria-hidden="true"></span>
				</button>
				<h2 class="hndle ui-sortable-handle"><span>Customer Order: </span></h2>
				<div class="inside">
				    	<div class="order-block">
						...
						<table class="wp-list-table widefat striped wccm-customer-details-table" >
								...
								<thead>
									<tr>
										<th><?php  _e('Order ID', 'woocommerce-customers-manager'); ?></th>
										<th><?php  _e('Product name', 'woocommerce-customers-manager'); ?></th>
										<th><?php  _e('Date', 'woocommerce-customers-manager'); ?></th>
										<th><?php  _e('Status', 'woocommerce-customers-manager'); ?></th>
								        <th><?php  _e('Employee', 'woocommerce-customers-manager'); ?></th>
								        <th><?php  _e('Subscription ID ', 'woocommerce-customers-manager'); ?></th>
										<th><?php  _e('Total', 'woocommerce-customers-manager'); ?></th>
										<th><?php  _e('Actions', 'woocommerce-customers-manager'); ?></th>
									</tr>
								</thead>
								<tbody>
								    <?php 
                        		    $orderss = $this->get_all_user_orders($this->current_customer_id); 
                        			...
                        			foreach($orderss as $order){
                        				...
                        				$products = $wc_order->get_items();
								  	    ...
								  	    global $wpdb;
                                        $subscriptionID = $wpdb->get_results( "SELECT ID FROM {$wpdb->prefix}posts where post_type='shop_subscription' AND post_parent = $order->ID  ");
                                        //echo "<pre>"; print_r($wc_order); echo "</pre>";
								  	    
            							foreach($products as $productn){ 
            							//echo"<pre>";print_r($data);
            								   ?>
							            <tr>
        										<td>  <a href="<?php echo get_edit_post_link( $order->ID ); ?>">
        												<?php echo $productn['order_id']; ?>
        											</a>  </td>
        										<td class="wccm_product_name_column"> <?php echo $productn['name']; ?></td>
        									
        										<td> <?php echo $order->post_date; ?> </td>
        										<td> <?php echo $status; ?> </td>
                            	                <td>
                                	                <?php 
                                    	                if( isset( 	$data['assigned_user_id'][0]) != '' ){
                                    						$user_info = get_userdata($data['assigned_user_id'][0]);
                                    						$user_name = $user_info->user_login ;
                                    					    echo $user_name;
                                    					}
                                					?>
                            					</td>
                            					<td><?php echo $subscriptionID[0]->ID; ?></td>
        										<td> <?php echo $productn['line_total']; //$wc_product->get_price_html(); ?> </td>
        										<td> <a class="button tips view" href="<?php echo get_edit_post_link( $order->ID ); ?>"><span class="dashicons dashicons-visibility"></span></a> </td>
									    </tr> 
									    <?php 
            						    }
									} 
	...
	<div id="postbox-container-3" class="postbox-container" style="display:none;">
		<div id="normal-sortables" class="meta-box-sortables ui-sortable">
				
			<div id="postorder_Data" class="postbox">
				<button type="button" id="order_btn" class="handlediv" aria-expanded="true">
					<span class="screen-reader-text">Alternar panel: Campos personalizados</span>
					<span class="toggle-indicator" aria-hidden="true"></span>
				</button>
				<h2 class="hndle ui-sortable-handle"><span>Orders</span></h2>
				<div class="inside">
				...
				<h4 id="wccm_order_list_title" class="remove_marborder"><?php _e('List for', 'woocommerce-customers-manager'); ?> <?php _e('period: ','woocommerce-customers-manager'); echo '<span class="wccm_woo_highlight">'.$this->starting_date. '</span> - <span class="wccm_woo_highlight">'.$this->ending_date."</span>"; ?></h4>
				<p id="wccm_order_description"><?php _e('To display orders for a <strong>different period</strong>, select a different <strong>date range in the Stat section</strong> (it is located after the map).', 'woocommerce-customers-manager'); ?></p>
				...
				<?php 
					...
					<div class="order-block">
						...
						?>
						<?php $data = get_post_custom( $order->ID ); ?>	<a class="button-primary" style="margin-top:10px;" href="<?php echo get_edit_post_link( $order->ID ); ?>"> <?php _e('Order details', 'woocommerce-customers-manager'); ?> </a>
						</span></span>
...
<div id="order-list-paging"></div>
	...
	<div id="postbox-container-2" class="postbox-container"style="display:none;">
		<div id="normal-sortables" class="meta-box-sortables ui-sortable">
			...
			<div id="postorder_chart" class="postbox">
				<button type="button" id="order_btnChart" class="handlediv" aria-expanded="true">
					<span class="screen-reader-text">Alternar panel: Campos personalizados</span>
					<span class="toggle-indicator" aria-hidden="true"></span>
				</button>
				<h2 class="hndle ui-sortable-handle"><span>Charts</span></h2>
				<div class="inside">
	...
	jQuery(document).ready(function($) {
		...
		jQuery('#customer_note').click(function(){
			
			var hasclass = jQuery('#postimagedivnote').hasClass('closed');
			if(hasclass){
				jQuery("#postimagedivnote").removeClass('closed');
			}else{
				jQuery("#postimagedivnote").addClass('closed');
			}
		...
		jQuery('#order_btn').click(function(){
			...
			var hasclass = jQuery('#postorder_Data').hasClass('closed');
			if(hasclass){
				jQuery("#postorder_Data").removeClass('closed');
			}else{
				jQuery("#postorder_Data").addClass('closed');
			}
	    	
		});
		jQuery('#order_btnChart').click(function(){
			
			var hasclass = jQuery('#postorder_chart').hasClass('closed');
			if(hasclass){
				jQuery("#postorder_chart").removeClass('closed');
			}else{
				jQuery("#postorder_chart").addClass('closed');
			}
	...
	static function get_all_users_with_orders_ids($current_page, $per_page, $customers_ids_to_use_as_filter = null, $get_total = false, $ids = null, $starting_date = null, $ending_date = null, $filter_by_product_id = null, $is_for_export = false)
	{
		global $wccm_customer_model;
		return $wccm_customer_model->get_all_users_with_orders_ids($current_page, $per_page, $customers_ids_to_use_as_filter, $get_total, $ids, $starting_date, $ending_date, $filter_by_product_id, $is_for_export);
	}
