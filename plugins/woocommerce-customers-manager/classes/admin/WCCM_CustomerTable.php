...
class WCCM_CustomerTable extends WP_List_Table {
    ...
    function column_login($item)
	{
       ...
       /*  return sprintf('%1$s <span style="color:silver">(id:%2$s)</span>%3$s',
		...
		return @sprintf( '<a href="?page='.$_REQUEST['page'].'&customer='.$item['ID'].'&action=customer_details">'.
							get_avatar($item['ID'], 64, "", false, array('class'=>'wccm_avatar_img')).'
								<div class="wccm_customer_data_container">
									<span class="wccm_name_surname_container">'.$item['name'].' '.$item['surname'].'</span>
									<span class="wccm_login_container">'.$item['login'].'</span>
								</div>
							</a>
							%1$s',
						$this->row_actions($actions));
	...
	function column_notes($item)
	{
		...
		$text = '<a class="wccm_tooltip">'.__('Click to read more', 'woocommerce-customers-manager').'<span>'.$item['notes'].'</span></a>';
		return $text;
	}
    ...
    function get_columns()
	{
        $columns = array(
			...
			'ID'     => 'ID',
			'login'     => __('Customer', 'woocommerce-customers-manager'),
			...
			'orders_list' => __('Action', 'woocommerce-customers-manager')
        );
					...
					//if($can_print_user)
						array_push($data, array( 'ID' => $customer->ID,
												...
												'orders_list' => /* '<a class="" target="_blank" href="'.admin_url('edit.php?s='.$customer->user_email.'&post_status=all&post_type=shop_order').'">'.
																	...
																	'</a>', */
																 '<a   data-toggle="tooltip" title="View Order" class="" target="_blank" href="'.admin_url('edit.php?s&post_status=all&post_type=shop_order&action=-1&_customer_user='.$customer->ID.'&filter_action=Filter').'">'.
																 '<span class="dashicons dashicons-list-view customeredit"></span>'.
																	/*'</a><a  class="" target="_blank"  data-toggle="tooltip" title="Create Order" href="'.admin_url('post-new.php?post_type=shop_order&action=-1&userids='.$customer->ID.'').'">'.
																 '<span class="dashicons dashicons-plus-alt customeredit"></span>'.*/
																 '</a><a class="custom_click" target="_blank"  data-toggle="tooltip" title="Create Order" href="'.home_url('/wp-login.php?action=logout&redirect_to=https%3A%2F%2Fwww.broomday.com%2Fnewdemo%2Flogin%3Fcustomer='.$customer->ID.'&_wpnonce=711f2163de').'">'.
										
																 '<span class="dashicons dashicons-plus-alt customeredit"></span>'.
																	'</a><a class="" target="_blank"  data-toggle="tooltip" title="View Customer" href="'.admin_url('?page=woocommerce-customers-manager&customer='.$customer->ID.'&action=customer_details').'">'.
																 '<span class="dashicons dashicons-edit customeredit"></span>'.
																	'</a>', 
												'address' => $address,						
