...
<div class="woocommerce_account_subscriptions">
	...
	<thead>
		<tr>
			...
			<th class="subscription-status order-status"><span class="nobr"><?php esc_html_e( 'Estado', 'woocommerce-subscriptions' ); ?></span></th>
			<!--<th class="subscription-next-payment order-date"><span class="nobr"><?php echo esc_html_x( 'Siguiente pago', 'table heading', 'woocommerce-subscriptions' ); ?></span></th>-->
			...
			<th class="subscription-next-booking next-booking"><span class="nobr"><?php echo esc_html_x( 'Fecha / hora de la próxima reserva', 'table heading', 'woocommerce-subscriptions' ); ?></span></th>
	...
	<?php foreach ( $subscriptions as $subscription_id => $subscription ) : ?>
		...
		<tr class="order">
			...
			<td class="subscription-status order-status" data-title="<?php esc_attr_e( 'Status', 'woocommerce-subscriptions' ); ?>">
				...
				<?php echo esc_html_e( wcs_get_subscription_status_name( $subscription->get_status() ) ); ?>
			</td>
			<!--<td class="subscription-next-payment order-date" data-title="<?php echo esc_attr_x( 'Next Payment', 'table heading', 'woocommerce-subscriptions' ); ?>">
				...
				<br/><small><?php echo apply_filters('woocommerce-subscriptions', $subscription->payment_method_title); ?></small>
				<?php endif; ?>
			</td>-->
			...
			<td class="subscription-total next-booking" data-title="<?php echo esc_attr_x( 'Next Booking Date/Time', 'Used in data attribute. Escaped', 'woocommerce-subscriptions' ); ?>">
				<?php if(function_exists('get_next_booking_datetime_for_subscription')){
					echo get_next_booking_datetime_for_subscription($subscription);
				}else{
					if($subscription->get_time( 'end' )!=0)echo spanish_date(date('F j, Y h:i a',strtotime($subscription->get_date( 'end' ))));?>
				<?php					
				}
				?>
			</td>
	...
	<?php else : ?>
		<p class="no_subscriptions">
			<?php if ( 1 < $current_page ) :
				...
				printf( esc_html__( 'No tienes suscripciones activas en este momento.', 'woocommerce-subscriptions' ), '<a href="' . esc_url( apply_filters( 'woocommerce_subscriptions_message_store_url', get_permalink( wc_get_page_id( 'shop' ) ) ) ) . '">', '</a>' );
			endif; ?>
		</p>
