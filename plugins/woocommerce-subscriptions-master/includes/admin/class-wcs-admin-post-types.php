...
class WCS_Admin_Post_Types {
	...
	public function __construct() {
		define('WCSS_PLUGIN_PATH', rtrim(plugin_dir_url(__FILE__), "/") );  	
		...
		add_action( 'restrict_manage_posts', array( $this, 'restrict_by_subscription' ) );
		...
		//wp_enqueue_style('wcss-common', WCSS_PLUGIN_PATH.'/css/common.css');
	}
	...
	public function shop_subscription_columns( $existing_columns ) {
		...
		$columns = array(
			...
			'customer_name'     => __( 'Customer Name', 'woocommerce-subscriptions' ),
			'frequency'    		=> __( 'Frequency', 'woocommerce-subscriptions' ),
...
//print_r($column );
		...
		switch ( $column ) {
			...
			//$column_content = sprintf( '<mark class="%s tips" data-tip="%s">%s</mark>', sanitize_title( $the_subscription->get_status() ), wcs_get_subscription_status_name( $the_subscription->get_status() ), wcs_get_subscription_status_name( $the_subscription->get_status() ) );
				...
				$all_statuses = array(
					'active'    => __( '<span class="dashicons dashicons-marker"></span>', 'woocommerce-subscriptions' ),
					'on-hold'   => __( '<span class="dashicons dashicons-migrate"></span>', 'woocommerce-subscriptions' ),
					'cancelled' => _x( '<span class="dashicons dashicons-dismiss"></span>', 'an action on a subscription', 'woocommerce-subscriptions' ),
					'trash'     => __( 'Trash', 'woocommerce-subscriptions' ),
					'deleted'   => __( 'Delete Permanently', 'woocommerce-subscriptions' ),
				);
...
//echo $post->ID;
				...
				$column_content .='<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) ) . '" class="edit"><span class="dashicons dashicons-edit customeredit"></span></a>';
				$column_content .='<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) ) . '" class="view"><span class="dashicons dashicons-visibility customeredit"></span></a>';
				
				//$column_content .= sprintf( _x( '%1$s#%2$s%3$s for %4$s', 'Subscription title on admin table. (e.g.: #211 for John Doe)', 'woocommerce-subscriptions' ), '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) ) . '">', '<strong>' . esc_attr( $the_subscription->get_order_number() ) . '</strong>', '</a>'  );
			...
			case 'customer_name' :
				...
				if ( $the_subscription->get_user_id() && ( false !== ( $user_info = get_userdata( $the_subscription->get_user_id() ) ) ) ) {
					...
					//$username  = '<a href="user-edit.php?user_id=' . absint( $user_info->ID ) . '">';
					//$username  = '<a href="' . esc_url( admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) ) . '">';
					$username  = '<a href="' . esc_url( admin_url( 'admin.php?page=woocommerce-customers-manager&customer=' . absint( $user_info->ID ) . '&action=customer_details' ) ) . '">';
			...
			$column_content .= $username;
			break;
			
			case 'frequency' :
			$column_content .= esc_html( strip_tags( $the_subscription->get_formatted_order_total_fre() ) );
			break;
	...
	public static function restrict_by_subscription() {
		global $typenow;
		...
		$getstatus = ( ! empty( $_GET['post_status'] ) ) ? $_GET['post_status'] : ''; ?>

		<select class="wcs_payment_method_selector first" name="post_status" id="post_status">
			<option value=""><?php esc_html_e( 'Any Status', 'woocommerce-subscriptions' ) ?></option>
			<option value="all_status" <?php echo esc_attr( 'all_status' == $getstatus ? 'selected' : '' ); ?>><?php echo esc_html__( 'All Status', 'woocommerce-subscriptions' ); ?></option>
            <option value="wc-active" <?php echo esc_attr( 'wc-active' == $getstatus ? 'selected' : '' ); ?>><?php echo esc_html__( 'Active', 'woocommerce-subscriptions' ); ?></option>
            <option value="wc-pending" <?php echo esc_attr( 'wc-pending' == $getstatus ? 'selected' : '' ); ?>><?php echo esc_html__( 'Pending ', 'woocommerce-subscriptions' ); ?></option>
            <option value="wc-on-hold" <?php echo esc_attr( 'wc-on-hold' == $getstatus ? 'selected' : '' ); ?>><?php echo esc_html__( 'On-Hold ', 'woocommerce-subscriptions' ); ?></option>
            <option value="wc-cancelled" <?php echo esc_attr( 'wc-cancelled' == $getstatus ? 'selected' : '' ); ?>><?php echo esc_html__( 'Cancelled ', 'woocommerce-subscriptions' ); ?></option>
		</select> <?php
	}
