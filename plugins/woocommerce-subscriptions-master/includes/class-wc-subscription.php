...
class WC_Subscription extends WC_Order {
			...
			try {
			 ...
			 $this->set_status( $new_status, $note, $manual );
				...
				switch ( $new_status ) {
					...
					break;
					case 'cancelled' :
						$dates_to_update['cancelled'] =$this->get_date( 'cancelled' );
	...
	public function get_date_to_display( $date_type = 'next_payment' ) {
		...
		if ( 'next_payment' == $date_type && $status != 'active' ) {
			$timestamp_gmt = 0;
	...
	public function get_formatted_order_total( $tax_display = '', $display_refunded = true ) {
 	...
 	public function get_formatted_order_total_fre($tax_display = '', $display_refunded = true)
	{
		return  $this->get_billing_interval().' '.$this->get_billing_period();
		if ( $this->get_total() > 0 && '' !== $this->get_billing_period() && ! $this->is_one_payment() ) {
			$formatted_order_total = wcs_price_string( $this->get_price_string_details( $this->get_total() ) );
		} else {
			$formatted_order_total = parent::get_formatted_order_total();
	...
	public function cancel_order( $note = '' ) {
		...
		$table = "wpstg2_postmeta";
		$post_id = $_GET['post'];
		
		$meta_value = date('Y-m-d H:i:s',strtotime("-1 days"));
		global $wpdb;
		$wpdb->update( $table, array("meta_value" => $meta_value), array("post_id"=>$post_id,"meta_key"=>"order_date"));
		//print_r(get_post_meta($post_id , "order_date"));die;
