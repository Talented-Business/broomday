...
class WC_Subscriptions_Payment_Gateways {
	...
	public static function trigger_gateway_renewal_payment_hook( $renewal_order ) {
		...
		if ( ! empty( $renewal_order ) && $renewal_order->get_total() > 0 && ! empty( $renewal_order_payment_method ) ) {
			...
			if ( ! isset( $available_gateways[ $renewal_order_payment_method ] ) ) {
				return;
			}
			
			WC()->session->set( 'order_awaiting_payment', $renewal_order->ID );
			$result = $available_gateways[ $renewal_order_payment_method ]->process_payment( $renewal_order->ID);
			
			
			if ( isset( $result['result'] ) && 'success' === $result['result'] ) {
			$result = apply_filters( 'woocommerce_payment_successful_result', $result, $renewal_order->ID );
			
			}*/
			do_action( 'woocommerce_scheduled_subscription_payment_' . $renewal_order_payment_method, $renewal_order->get_total(), $renewal_order );
			//do_action( 'woocommerce_scheduled_subscription_payment_', $renewal_order->get_total(), $renewal_order );
		}
	}
