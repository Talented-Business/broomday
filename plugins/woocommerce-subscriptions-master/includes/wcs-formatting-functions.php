...
function wcs_price_string( $subscription_details ) {
	...
	if ($subscription_details['subscription_period'] == "week")
	{
			$subscription_details['subscription_period'] = "week";
	}
	...
	if ( $subscription_details['subscription_length'] > 0 && $subscription_details['subscription_length'] == $subscription_details['subscription_interval'] ) {
		...
		if ( true === $subscription_details['use_per_slash'] ) {
			...
			$subscription_string = sprintf( _n( '%1$s / Semana', '%1$s cada %2$s', $subscription_details['subscription_interval'], 'woocommerce-subscriptions' ), $recurring_amount_string, $subscription_period_string );
			$subscription_string = str_replace("weeks","semanas",$subscription_string);
