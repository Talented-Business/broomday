...
function wc_cart_totals_subtotal_html() {
	echo WC()->cart->get_cart_subtotal($compound = false); // WPCS: XSS ok.
}
 ...
 */
function wc_cart_totals_order_total_html($product=false) {
	$value = '<strong>' . WC()->cart->get_total('view', $product) . '</strong> ';
