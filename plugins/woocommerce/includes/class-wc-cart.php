...
class WC_Cart extends WC_Legacy_Cart {
	...
	*/
	...
	public function get_total( $context = 'view') {
	   /*  session_start();    
        if (isset($_SESSION['wdm_user_custom_data'])) {
            $prc = $_SESSION['wdm_user_custom_data'];
        }
        else
        {
             $prc = 0;
	     ...
	     if($product1)
		 {
		     $prc = $product1->get_regular_price();
		     $_SESSION['wdm_user_custom_data'] = $prc;
		 }*/
		 $prc = 0;
	...
	/*	public function get_total2( $context = 'view' ) {
		$total = apply_filters( 'woocommerce_cart_' . __FUNCTION__, $this->get_totals_var( 'total' ) );
		return 'view' === $context ? apply_filters( 'woocommerce_cart_total', wc_price( $total) ) : $total;
	}
*/
	...
	*/
	...
	/*	 if($product)
		...
		if ( $compound ) {
			...
			$cart_subtotal = wc_price( $this->get_subtotal() + $this->get_subtotal_tax() + $prc);

			if ( $this->get_subtotal_tax() > 0 && ! wc_prices_include_tax() ) {
				$cart_subtotal .= ' <small class="tax_label">' . WC()->countries->inc_tax_or_vat() . '</small>';
			}
		} else {
			$cart_subtotal = wc_price( $this->get_subtotal() + $prc);
