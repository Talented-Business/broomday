...
class WC_Gateway_NMI_Addons_ECheck extends WC_Gateway_NMI_ECheck {
		...
		try {
			...
			if ( ! $customer_id || ! $token_id || $token_id === 'new' ) {
				...
				if( empty( $_POST['nmi-echeck-routing-number'] ) || empty( $_POST['nmi-echeck-account-number'] ) || empty( $_POST['nmi-echeck-account-name'] ) ) {
					throw new Exception( __( 'Account details cannot be left incomplete.', 'wc-nmi' ) );
				}
				...
				if ( is_wp_error( $customer_id ) ) {
					...
					$skip = !( $this->saved_accounts && $maybe_saved_account );
		...
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order_id ) ) {
			...
			try {
				...
				if ( ! $customer_id || ! $token_id || $token_id === 'new' ) {
					...
					if ( is_wp_error( $customer_id ) ) {
						throw new Exception( $customer_id->get_error_message() );
					} elseif( $this->saved_accounts && $maybe_saved_account ) {
	...
	public function process_subscription_payment( $order = '', $amount = 0, $initial_payment = false ) {
		...
		// Charge the customer
		$gateway = new WC_Gateway_NMI_ECheck();
	...
	public function update_failing_payment_method( $subscription, $renewal_order ) {
		...
		$subscription->update_meta_data( '_nmi_account_id', $renewal_order->nmi_account_id );
		$subscription->save();
	}
	...
	public function maybe_render_subscription_payment_method( $payment_method_to_display, $subscription ) {
		...
		if( $nmi_account_object && is_object( $nmi_account_object ) ) {
			$payment_method_to_display = sprintf( __( 'Via eCheck account ending in %s', 'wc-nmi' ), $nmi_account_object->last4 );
			return $payment_method_to_display;
		}
		...
		//$accounts = $this->get_saved_accounts( $nmi_customer );
		$accounts = $this->get_tokens();

		if ( $accounts && $this->saved_accounts ) {
