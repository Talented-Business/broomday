...
class WC_NMI_Privacy extends WC_Abstract_Privacy {
	...
	public function __construct() {
		...
		if ( function_exists( 'wcs_get_subscriptions' ) ) {
			$this->add_exporter( 'wc-nmi-subscriptions-data', __( 'WooCommerce Network Merchants (NMI) Subscriptions Data', 'wc-nmi' ), array( $this, 'subscriptions_data_exporter' ) );
		}

		$this->add_exporter( 'wc-nmi-customer-data', __( 'WooCommerce Network Merchants (NMI) Customer Data', 'wc-nmi' ), array( $this, 'customer_data_exporter' ) );

		$this->add_eraser( 'wc-nmi-customer-data', __( 'WooCommerce Network Merchants (NMI) Customer Data', 'wc-nmi' ), array( $this, 'customer_data_eraser' ) );
	...
	public function account_settings( $settings ) {
		$insert_setting = array(
			array(
				...
				'desc_tip'    => __( 'Retains any Network Merchants (NMI) data such as Network Merchants (NMI) customer ID, charge ID.', 'wc-nmi' ),
			...
			foreach ( $orders as $order ) {
				$data_to_export[] = array(
					...
					'data'        => array(
						...
						array(
							'name'  => __( 'Network Merchants (NMI) customer id', 'wc-nmi' ),
	...
	public function customer_data_eraser( $email_address, $page ) {
		...
		if ( ! empty( $nmi_customer_id ) ) {
			...
			$messages[] = __( 'Network Merchants (NMI) User Data Erased.', 'wc-nmi' );
		}
	...
	protected function maybe_handle_subscription( $order ) {
		...
		if ( $subscription->has_status( apply_filters( 'wc_nmi_privacy_eraser_subs_statuses', array( 'on-hold', 'active' ) ) ) ) {
			return array( false, true, array( sprintf( __( 'Order ID %d contains an active Subscription. Personal data retained. (Network Merchants (NMI))', 'wc-nmi' ), $order->get_id() ) ) );
		}
		...
		return array( true, false, array( __( 'Network Merchants (NMI) Subscription Data Erased.', 'wc-nmi' ) ) );
	}
